...
data_cv_rf<-vfold_cv(train_classifiers, v = 10)

rf_recipe<-recipe(PSS10_stress_dico ~ ., data = train_classifiers)

rf_spec<-rand_forest(
  mtry = tune(), #umber of predictors that will be randomly sampled at each split
...
rf_tune_grid <- grid_regular(mtry(range = c(1,ncol(modelling_phase_data) - 1)), trees(), levels = 10)
...
rf_grid <- tune_grid(rf_spec,
                     rf_recipe,
                     resamples = data_cv_rf,
                     grid = rf_tune_grid)

toc()

rf_lowest_roc_auc <- rf_grid %>%
  select_best("roc_auc")
...
rf_model<-finalize_workflow(
  wf_rf %>% add_model(rf_spec),
  rf_lowest_roc_auc) %>%
  fit(data = train_classifiers)


prediction_class_rf_train<-predict(rf_model, train_classifiers, type = "class")

prediction_prob_rf_train<-predict(rf_model, train_classifiers, type = "prob")


metrics_rf_data_train<-train_classifiers %>%
  select(PSS10_stress_dico) %>%
  bind_cols(prediction_class_rf_train, prediction_prob_rf_train)
...
threshold_prob_rf<-metrics_rf_data_train %>%
...
max_j_index_threshold_prob_rf <- threshold_prob_rf %>%
  filter(.metric == "j_index") %>%
  filter(.estimate == max(.estimate)) %>%
  pull(.threshold) %>%
  min()

j_index_plot_rf<-ggplot(threshold_prob_rf, aes(x = .threshold, y = .estimate, color = .metric, alpha = group)) +
  geom_line() +
  theme_minimal() +
  scale_color_viridis_d(end = 0.9) +
  scale_alpha_manual(values = c(.4, 1), guide = "none") +
  geom_vline(xintercept = max_j_index_threshold_prob_rf, alpha = .6, color = "grey30") +
  labs(
    x = "Alto estrés",
    y = "Metrica",
...
metrics_rf_data_train_new_threshold<-metrics_rf_data_train %>% 
  mutate(`.pred_class` = make_two_class_pred(`.pred_high stress`, 
                                             levels(metrics_rf_data_train$PSS10_stress_dico),
                                             threshold = max_j_index_threshold_prob_rf),
         `.pred_class` = factor(.pred_class, levels = levels(metrics_rf_data_train$PSS10_stress_dico)),
...
names(metrics_rf_data_train_new_threshold)<-c("outcomes", "predicted outcomes","predictions")

#calibration model
other_metrics_rf_train<-rms::val.prob(metrics_rf_data_train_new_threshold$predictions, metrics_rf_data_train_new_threshold$outcomes)
other_metrics_rf_train<-as.data.frame(other_metrics_rf_train)
other_metrics_rf_train$metric<-rownames(other_metrics_rf_train)

calibrated_prob_rf<-stdRecal(metrics_rf_data_train_new_threshold$outcomes, metrics_rf_data_train_new_threshold$predictions)

metrics_rf_data_train_new_threshold$`calibrated predictions`<-calibrated_prob_rf$p.std

#calibration test set
prediction_class_rf_test<-predict(rf_model, test_classifiers, type = "class")

prediction_prob_rf_test<-predict(rf_model, test_classifiers, type = "prob")

metrics_rf_data_test<-test_classifiers %>%
  select(PSS10_stress_dico) %>%
  bind_cols(prediction_class_rf_test, prediction_prob_rf_test) %>%
  select(-`.pred_normal stress`) %>%
  mutate(PSS10_stress_dico = ifelse(PSS10_stress_dico == "high stress", 1, 0),
         `.pred_high stress` = exp(-2.595953 + 14.46795 * log(`.pred_high stress`/(1-`.pred_high stress`)))/(1+exp(-2.595953 + 14.46795 * log(`.pred_high stress`/(1-`.pred_high stress`)))),
         .pred_class = ifelse(`.pred_high stress` >= max_j_index_threshold_prob_rf, 1, 0))

names(metrics_rf_data_test)<-c("outcomes", "predicted outcomes","predictions")


#metrics of discrimination
conf_mat_rf_results_train<-table(metrics_rf_data_train_new_threshold$`predicted outcomes`, #columns
                                       metrics_rf_data_train_new_threshold$outcomes) #rows
conf_mat_rf_results_train<-conf_mat_rf_results_train[2:1,2:1]

conf_mat_rf_results_test<-table(metrics_rf_data_test$`predicted outcomes`, #columns
                                      metrics_rf_data_test$outcomes) #rows
conf_mat_rf_results_test<-conf_mat_rf_results_test[2:1,2:1]


#discrimination metrics train
discrim_metrics_rf_train<-epi.tests(conf_mat_rf_results_train)
sens_rf_train<-discrim_metrics_rf_train$detail$se
spec_rf_train<-discrim_metrics_rf_train$detail$sp
pvp_rf_train<-discrim_metrics_rf_train$detail$pv.pos
pvn_rf_train<-discrim_metrics_rf_train$detail$pv.neg
discrim_metrics_rf_train<-rbind.data.frame(round(sens_rf_train, 3), 
                                                 round(spec_rf_train, 3),
                                                 round(pvp_rf_train, 3), 
                                                 round(pvn_rf_train, 3)) %>%
...
roc_rf_train<-pROC::roc(metrics_rf_data_train_new_threshold$outcomes, metrics_rf_data_train_new_threshold$`calibrated predictions`)
ci_roc_rf_train<-pROC::ci.auc(roc_rf_train)


#discrimination metrics test  
discrim_metrics_rf_test<-epi.tests(conf_mat_rf_results_test)
sens_rf_test<-discrim_metrics_rf_test$detail$se
spec_rf_test<-discrim_metrics_rf_test$detail$sp
pvp_rf_test<-discrim_metrics_rf_test$detail$pv.pos
pvn_rf_test<-discrim_metrics_rf_test$detail$pv.neg
discrim_metrics_rf_test<-rbind.data.frame(round(sens_rf_test, 3), 
                                                round(spec_rf_test, 3),
                                                round(pvp_rf_test, 3), 
                                                round(pvn_rf_test, 3)) %>%
  mutate(medida = c("sensibilidad", "especificidad", "valor predictivo positivo","valor predictivo negativo"),
         prueba =  paste0(est, "(IC 95% ", lower,"-", upper, ")")) %>%
  select(medida, prueba)
roc_rf_test<-pROC::roc(metrics_rf_data_test$outcomes, metrics_rf_data_test$predictions)
ci_roc_rf_test<-pROC::ci.auc(roc_rf_test)

#discrimination metrics rf
discrim_metrics_rf<-full_join(discrim_metrics_rf_train, discrim_metrics_rf_test, by = "medida")

discrim_metrics_rf<-discrim_metrics_rf %>% add_case(medida = "ROC", 
                                                                entrenamiento = paste0(round(ci_roc_rf_train[2],3), "(IC 95% ", round(ci_roc_rf_train[1],3),"-", round(ci_roc_rf_train[3],3), ")"),
                                                                prueba = paste0(round(ci_roc_rf_test[2],3), "(IC 95% ", round(ci_roc_rf_test[1],3),"-", round(ci_roc_rf_test[3],3), ")"))

#calibration and general performance metrics training set
other_metrics_rf_train_cal<-rms::val.prob(metrics_rf_data_train_new_threshold$`calibrated predictions`, metrics_rf_data_train_new_threshold$outcomes)
other_metrics_rf_train_cal<-as.data.frame(other_metrics_rf_train_cal)
other_metrics_rf_train_cal$metric<-rownames(other_metrics_rf_train_cal)
other_metrics_rf_train_cal<-other_metrics_rf_train_cal %>% 
  filter(metric == "Intercept" | metric == "Slope" | metric == "Brier")
names(other_metrics_rf_train_cal)<-c("entrenamiento", "medida")


#calibration and general performance metrics testing set
other_metrics_rf_test<-rms::val.prob(metrics_rf_data_test$predictions, metrics_rf_data_test$outcomes)
other_metrics_rf_test<-as.data.frame(other_metrics_rf_test)
other_metrics_rf_test$metric<-rownames(other_metrics_rf_test)
other_metrics_rf_test<-other_metrics_rf_test %>% 
  filter(metric == "Intercept" | metric == "Slope" | metric == "Brier")
names(other_metrics_rf_test)<-c("prueba", "medida")

#calibration metrics rf
calibration_metrics_rf<-full_join(other_metrics_rf_train_cal, other_metrics_rf_test, by = "medida")
calibration_metrics_rf$entrenamiento<-round(calibration_metrics_rf$entrenamiento, 3)
calibration_metrics_rf$prueba<-round(calibration_metrics_rf$prueba, 3)

#performance rf
performance_rf<-rbind.data.frame(discrim_metrics_rf, calibration_metrics_rf)
...
#----------
#metrics train data
dca_rf_train<-dca(data = metrics_rf_data_train_new_threshold,
                        outcome = "outcomes",
...
netbenef_rf_train_cal_df<-dca_rf_train$net.benefit

netbenf_rf_train_cal<-netbenef_rf_train_cal_df %>%
  filter(threshold == 0.553) %>%
  select(`calibrated predictions`) %>%
  as.numeric() %>%
  round(3) %>%
  as.character()

netbenef_rf_train_cal_diff<-netbenef_rf_train_cal_df %>%
  filter(threshold == 0.553) %>%
...
netbenef_rf_train_cal_interv_avoided<-dca_rf_train$interventions.avoided %>%
  filter(threshold == 0.553) %>%
  select(`calibrated predictions`) %>%
  as.numeric() %>%
  round(3) %>%
...
#metrics test data
dca_rf_test<-dca(data = metrics_rf_data_test,
                       outcome = "outcomes",
...
netbenef_rf_test_df<-dca_rf_test$net.benefit

netbenf_rf_test<-netbenef_rf_test_df %>%
  filter(threshold == 0.553) %>%
  select(predictions) %>%
  as.numeric() %>%
  round(3) %>%
...
netbenef_rf_test_diff<-netbenf_rf_test_cal<-netbenef_rf_test_df %>%
  filter(threshold == 0.553) %>%
...
netbenef_rf_test_interv_avoided<-dca_rf_test$interventions.avoided %>%
  filter(threshold == 0.553) %>%
  select(predictions)%>%
  as.numeric() %>%
  round(3) %>%
...
performance_rf<-performance_rf %>%
  add_case(medida = c("Beneficio neto", "Beneficio neto (evaluar a todos vs modelo)", "Evaluaciones evitadas"),
           entrenamiento = c(netbenf_rf_train_cal, netbenef_rf_train_cal_diff, netbenef_rf_train_cal_interv_avoided),
           prueba = c(netbenf_rf_test, netbenef_rf_test_diff, netbenef_rf_test_interv_avoided))

names(performance_rf)<-c("medida", "entrenamiento rf", "prueba rf")
